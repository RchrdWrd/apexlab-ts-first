...
function App() {
  ...
  const [movieData, setMovieData] = React.useState<ImovieData>({
    name: "",
    score: 0,
    genres: [],
    imgPoster: "",
  ...
  async function searchMovies() {
    if (searchValue === null || searchValue.match(/^ *$/) !== null)
      ...
      await setOptions(
        JSON.parse(JSON.stringify(md.getResponse()).replace(/\:null/gi, ':""'))
          .data.searchMovies
      );
                            ...
                            clickAction={() => {
                              ...
                              getSummary(data.name).then((summary) => {
                                setMovieData({
                                  ...
                                  score: data.score,
                                  genres: data.genres,
                                  imgPoster: data.imgPoster.url,
                                  imgBackdrop: data.imgBackdrop.original,
                                  summary: summary,
                                });
                              });
                            }}
        ...
        >
          <Box className={AppStyle.MovieData}>
            <MovieDataBox
              ...
              openWIKI={() => {
                window.open(
                  `https://en.wikipedia.org/wiki/${movieData.name}`,
                  "_blank"
                );
              }}
            ></MovieDataBox>
